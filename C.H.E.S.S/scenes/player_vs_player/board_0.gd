...
func setup_ui():
	...
	# We'll use White pieces for the icons by default
	var options = ["q", "r", "b", "n"] 
	for type in options:
		var btn = Button.new()
...
func select_piece(pos: Vector2i):
	...
	for target in valid_moves:
		...
		# Use get_slice_texture for moves and captures
		if is_capture:
...
func spawn_move_highlight(grid_pos: Vector2i):
	...
	# Create the frames dynamically using your loaded textures
	var frames = SpriteFrames.new()
	frames.add_animation("default")
	frames.set_animation_loop("default", true)
	frames.set_animation_speed("default", 2.0) # 2 FPS = 0.5 seconds per frame
	
	# Add the two textures you wanted to swap between
	frames.add_frame("default", get_slice_texture("moved"))
	frames.add_frame("default", get_slice_texture("moved_"))
	
	s.sprite_frames = frames
	s.play("default")
	s.centered = true # Important if your textures are top-left aligned like the board
	
	# Position logic (matching your grid_to_pixel helper)
...
func spawn_highlight(highlight_texture, grid_pos):
	if highlight_texture == null: return # Safety check
	var s = Sprite2D.new()
...
func update_last_move_visuals(start: Vector2i, end: Vector2i):
	...
	# 2. Spawn new animated highlights for both Start and End positions
